...
{
    [ECAuth()]
    public class WellPIP2Controller : Controller
    {
        // GET: Shell/AdminWell
                ...
                if (wells.Count > 0)
                {
                    foreach (var x in wells)
                    {
                        ...
                        ret.AddRange(WellPIP.Populate<WellPIP>(query,
                            fields: new string[] { "WellName", "SequenceId", "Version", "Status", "ActivityType", "_id", "PhaseNo","Type" }));
                ...
                else
                {
                    if (!NoneWellMatchWithEXType.Trim().Equals("NoneWellMatchWithEXType"))
                    {
                        ...
                        ret = WellPIP.Populate<WellPIP>(queryForPIP,
                            fields: new string[] { "WellName", "SequenceId", "Status", "Version", "ActivityType", "_id", "PhaseNo", "Type" });
                    }
                    else
                    {
        ...
        public JsonResult Get(object id)
        {
            ...
            #region GetPIPElementWithLoFValues

            var wpLoE = PIPElementLoE.GetPIPElementWithLoFValues(id.ToString());
            if (wpLoE != null && wpLoE.Count > 0)
            {
                foreach(var w in wp.Elements){
                    var getLoE = wpLoE.Where(x => x.ElementId == w.ElementId);
                    if (getLoE != null)
                    {
                        w.LevelOfEstimate = getLoE.FirstOrDefault().LevelOfEstimate;
                    }
                }
            }
            ...
            return Json(new { Success = true, Data = wp, WAU = wau, WPLoE = wpLoE }, JsonRequestBehavior.AllowGet);
        }

        public JsonResult BulkSaveLevelOfEstimate(string PIPId, List<int> ElementIds, int LoE)
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                //var updateLoE = PIPElementLoE.BulkSaveLevelOfEstimate(PIPId, ElementIds, LoE, WebTools.LoginUser.UserName);
                var updateLoE = new PIPElementLoE();
                updateLoE.BulkSaveLevelOfEstimate(PIPId, ElementIds, LoE, WebTools.LoginUser.UserName);
                return "OK";
            });
        }
        ...
        public JsonResult UpdatePIP(string PIPId, List<PIPElement> updated)
        {
            try
            {
                ...
                {
                    //get allocation from PIP --> because of Period issue
                    var alloc = wp.Elements.Where(x => x.ElementId == y.ElementId).FirstOrDefault().Allocations;

                    y.Allocations = alloc;
                    var u = PIPElementLoE.TransformElementToLOE(y, PIPId, WebTools.LoginUser.UserName);
                    u.Save();
                }
        ...
        public JsonResult GetDataAllocation(string PIPId, int ElementId)
        {
            try
            {
                ...
                var DataLoEs = PIPElementLoE.GetPIPElementLoEs(PIPId,ElementId);

                return Json(new { Success = true, Data = Elements.Allocations.ToArray(), monthDiff = mthNumber,DataLoEs = DataLoEs }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception e)
            {
                return Json(new { Success = false, Message = e.Message }, JsonRequestBehavior.AllowGet);
        ...
        public JsonResult GetOlderDataAllocation(string PIPId, int ElementId, int LoE)
        {
            try
            {

                var DataLoEs = PIPElementLoE.GetPIPElementLoEs(PIPId, ElementId);
                var DataAlloc = DataLoEs.Where(x => x.LevelOfEstimate.Equals(LoE)).FirstOrDefault();
                var Allocation = new List<PIPAllocation>();
                if (DataAlloc != null)
                {
                    Allocation = DataAlloc.Allocations;
                }

                return Json(new { Success = true, Data = Allocation.ToArray() }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception e)
            {
                return Json(new { Success = false, Message = e.Message }, JsonRequestBehavior.AllowGet);
            ...
            List<PIPElement> PIPs = null)
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                ...
                var OP = DayOrCost.Equals("Day") ? act.OP.Days : act.OP.Cost;
        ...
        public JsonResult GetWaterfallChartLevel0(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                qs.Add(Query.EQ("WellName", WellName));
                qs.Add(Query.EQ("ActivityType", ActivityType));
                var PIPDetail = WellPIP.Get<WellPIP>(Query.And(qs));

                string SequenceId = PIPDetail.SequenceId;

                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                var getData = WaterfallLvl0Estimate.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }

        public JsonResult GetWaterfallChartLevel3dg3(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                ...
                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                var getData = WaterfallDG3Estimate.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }

        public JsonResult GetWaterfallChartLevel4AFE(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                ...
                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                var getData = WaterfallAFEEstimate.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }

        public JsonResult GetWaterfallChartExecutionEstimate(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                ...
                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                var getData = WaterfallExecutionEstimate.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }
        public JsonResult GetWaterfallChartFinal(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                ...
                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                var getData = WaterfallFinal.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }
        public JsonResult GetWaterfallChartCostSavingAnalysis(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                ...
                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                //var getData = WaterfallFinal.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                var getData = WaterfallAnalysis.LoadData(GroupBy, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }

        public JsonResult GetWaterfallChartExecutionEstimate2(string WellName, string ActivityType, string GroupBy = "Classification", string DayOrCost = "Day")
        {
            return MvcResultInfo.Execute(null, (BsonDocument doc) =>
            {
                var qs = new List<IMongoQuery>();
                ...
                var PIPElementHist = WellPIPElementHistory.GetLOV(WellName, SequenceId, ActivityType);
                //var getData = WaterfallAFEEstimate.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                var getData = WaterfallExecutionEstimate2.LoadData(GroupBy, DayOrCost, WellName, SequenceId, ActivityType, PIPElementHist);
                return getData;
            });
        }
