...
namespace ECIS.AppServer.Areas.Shell.Controllers
{
    ...
    public class UploadOPController : Controller
    {
        ...
        public JsonResult FixMeta(string id)
        {
            ...
            try
            {
                ...
                var waqs = new List<IMongoQuery>();
                foreach (KeyValuePair<string, string> key in keys)
                    waqs.Add(Query.EQ(key.Key, BsonNull.Value));
                var waq = Query.Or(waqs.ToArray());

                var tableName = new WellActivity().TableName;

                WellActivity.Populate<WellActivity>(waq).ForEach(d =>
                {
                    var doc = d.ToBsonDocument();
                    var should = datas.Where(f =>
                    {
                        var isExists = true;
                        foreach (KeyValuePair<string, string> key in keys)
                        {
                            if (doc.Get(key.Key) != BsonNull.Value)
                            {
                                isExists &= f.GetString(key.Value).Equals(doc.GetString(key.Key));
                            }
                        }
                        return isExists;
                    });
                    ...
                    if (should.Any())
                    {
                        var shouldDoc = should.FirstOrDefault();
                        foreach (KeyValuePair<string, string> key in keys)
                        {
                            doc.Set(key.Key, shouldDoc.GetString(key.Value));
        ...
        public JsonResult Execute(string id)
        {
            ...
            try
            {
                ...
                // --- Prepare for check master data in current data
                var cuurentWellPlan = new List<WellActivity>();
                foreach (var y in currentDB)
                {
                    cuurentWellPlan.Add(BsonSerializer.Deserialize<WellActivity>(y));
                }
                // --- end 
                ...
                DataHelper.Delete("WEISWellActivityPhaseInfos");
                ...
                foreach (var t in wellActvNew)
                {
                    ...
                    {
                        var ada = cuurentWellPlan.Where(x => x.WellName.Trim().Equals(t.WellName.Trim()));
                        if(ada.Any())
                        {
                            t.ProjectName = ada.FirstOrDefault().ProjectName;
                            t.Region = ada.FirstOrDefault().Region;
                            t.RigType = ada.FirstOrDefault().RigType;
                            t.OperatingUnit = ada.FirstOrDefault().OperatingUnit;
                            t.PerformanceUnit = ada.FirstOrDefault().PerformanceUnit;
                            t.AssetName = ada.FirstOrDefault().AssetName;
                        }
                    }
                ...
                TestController.SaveInitialEmpytMaster();

                var regions = wellActvNew.GroupBy(x => x.Region).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in regions)
                {
                    DataHelper.Save("WEISRegions", new BsonDocument("_id", r));
                }

                var rigTypes = wellActvNew.GroupBy(x => x.RigType).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in rigTypes)
                {
                    DataHelper.Save("WEISRigTypes", new BsonDocument("_id", r));
                }

                var rigNames = wellActvNew.GroupBy(x => x.RigName).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in rigNames)
                {
                    DataHelper.Save("WEISRigNames", new BsonDocument("_id", r));
                }

                var operatingUnits = wellActvNew.GroupBy(x => x.OperatingUnit).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in operatingUnits)
                {
                    DataHelper.Save("WEISOperatingUnits", new BsonDocument("_id", r));
                }

                var performanceUnits = wellActvNew.GroupBy(x => x.PerformanceUnit).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in performanceUnits)
                {
                    DataHelper.Save("WEISPerformanceUnits", new BsonDocument("_id", r));
                }

                var assetNames = wellActvNew.GroupBy(x => x.AssetName).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in assetNames)
                {
                    DataHelper.Save("WEISAssetNames", new BsonDocument("_id", r));
                }

                var projectNames = wellActvNew.GroupBy(x => x.ProjectName).Select(x => x.Key).Where(d => d != null).ToList();
                foreach (var r in projectNames)
                {
                    DataHelper.Save("WEISProjectNames", new BsonDocument("_id", r));
                }
                ...
                try { new TestController().SyncActivityCategory(); }
                catch (Exception ez) {  }
        ...
        public JsonResult LoadCollection()
        {
            try
            {
                ...
                var listBackupWA = DataHelper.Populate("BackupListWellActivity");
                foreach (string y in t)
                {
                    ...
                    var sg = listBackupWA.FirstOrDefault(x => x.GetString("CollectionName").Equals(y));
                    BsonDocument d = new BsonDocument();
                    d.Set("CollectionName", y.ToString());
                    d.Set("CreateDate", Tools.ToUTC(dt));
                    d.Set("User", sg != null ? sg.GetString("User") : "-");
