...
{
    [ECAuth()]
    public class DataModelSPController : Controller
    {
        ...
        public JsonResult UpdatePhase2(int id, string wellname, string rigname, List<WellActivityPhase> updatedPhases, List<WellActivityPhaseInfo> updatePhaseInfo, WellActivity updateActivity)
        {
            return MvcResultInfo.Execute(null, (BsonDocument d) =>
            {
                ...
                var originalActivity = WellActivity.Get<WellActivity>(Query.And(qs));
        ...
        public ActionResult DeletePhase(int id, int PhaseNo, string ActivityType = "")
        {
            try
            {
                ...
                if (wa.Phases.Any())
                {
                    ...
                    {
                        if (!i.PhaseNo.Equals(PhaseNo) && !i.ActivityType.Equals(ActivityType))
                        {
                            wap.Add(i);
                        }
                    }
        ...
        public JsonResult GetWellSequenceInfoSp(WaterfallBase wb, int take = 10, int skip = 0, List<Dictionary<string, string>> sorts = null)
        {
            ResultInfo ri = new ResultInfo();
            try
            {
                ...
                int coutdata = 0;
                var wa = fh.GetActivities(wb, out coutdata, take, skip, WebTools.LoginUser.Email, serversorts: sorts);
                stw.Stop();
                var ss = stw.Elapsed.TotalSeconds;
                
                List<ECIS.Client.WEIS.Output.DataBrowserOut> ListdbOut = new List<ECIS.Client.WEIS.Output.DataBrowserOut>();
                foreach (var d in wa)
                {
                    var dbOut = new ECIS.Client.WEIS.Output.DataBrowserOut();
                    ...
                    if (d.Phases.Any())
                    {
                        foreach (var ph in d.Phases)
                        {
                            ...
                            if (ph.BaseOP.Contains(DefaultOP))
                            {
                                ...
                                dbOut.PlanDuration += ph.Plan.Days;
                                dbOut.PlanCost += ph.Plan.Cost;
                    ...
                    dbOut._id = d._id;
                    dbOut.Region = d.Region;
                    dbOut.OperatingUnit = d.OperatingUnit;
                    dbOut.UARigSequenceId = d.UARigSequenceId;
                    dbOut.RigType = d.RigType;
                    dbOut.RigName = d.RigName;
                    dbOut.ProjectName = d.ProjectName;
                    dbOut.WellName = d.WellName;
                    dbOut.AssetName = d.AssetName;
                    dbOut.NonOP = d.NonOP;
                    dbOut.WorkingInterest = d.WorkingInterest;
                    dbOut.IsInPlan = listPlan;
                    dbOut.PsDuration = d.PsSchedule != null ? d.PsSchedule.Days : 0;
                    dbOut.PlanDuration = PlanDuration;
                    dbOut.PlanCost = Tools.Div(PlanCost, division);
                    dbOut.PreviousPlanDuration = PreviousPlanDuration;
                    dbOut.PreviousPlanCost = Tools.Div(PreviousPlanCost, division);
                    dbOut.OpsCost = Tools.Div(OpsCost, division);
                    dbOut.OpsDuration = OpsDuration;
                    dbOut.OpsStart = d.OpsSchedule != null ? d.OpsSchedule.Start : Tools.DefaultDate;
                    dbOut.OpsFinish = d.OpsSchedule != null ? d.OpsSchedule.Finish : Tools.DefaultDate;
                    dbOut.PhDuration = (d.Phases.Count() > 0 ? d.Phases.Sum(x => x.PhSchedule == null ? 0 : x.PhSchedule.Days) : 0);
                    dbOut.UARigDescription = String.Format("{0} - {1}", d.UARigSequenceId, d.UARigDescription == null ? "" : d.UARigDescription);
                    dbOut.PhRiskDuration = (d.Phases.Count() > 0 ? d.Phases.Where(x => x.RiskFlag == null ? false : !x.RiskFlag.Equals("")).Sum(x => x.PhSchedule == null ? 0 : x.PhSchedule.Days) : 0);
                    dbOut.PhStartForFilter = (d.Phases.Count() > 0 ? d.Phases.Min(e => e.PhSchedule == null ? Tools.DefaultDate : e.PhSchedule.Start) : Tools.DefaultDate);
                    dbOut.PhFinishForFilter = (d.Phases.Count() > 0 ? d.Phases.Max(e => e.PhSchedule == null ? Tools.DefaultDate : e.PhSchedule.Finish) : Tools.DefaultDate);
                    dbOut.PsStartForFilter = (d.PsSchedule == null ? Tools.DefaultDate : d.PsSchedule.Start);
                    dbOut.PsFinishForFilter = (d.PsSchedule == null ? Tools.DefaultDate : d.PsSchedule.Finish);
                    dbOut.PsStart = PsStart;
                    dbOut.PsFinish = PsFinish;
                    dbOut.PreviousPsStart = PreviousPsStart;
                    dbOut.PreviousPsFinish = PreviousPsFinish;
                    dbOut.PhCost = (d.Phases.Count() > 0 ? (d.Phases.Sum(e => e.OP.Cost) / division) : 0);
                    dbOut.AFEStart = (d.Phases.Count() > 0 ? d.Phases.Min(e => e.AFESchedule == null ? Tools.DefaultDate : e.AFESchedule.Start) : Tools.DefaultDate);
                    dbOut.AFEFinish = (d.Phases.Count() > 0 ? d.Phases.Max(e => e.AFESchedule == null ? Tools.DefaultDate : e.AFESchedule.Finish) : Tools.DefaultDate);
                    dbOut.AFEDuration = (d.Phases.Count() > 0 ? d.Phases.Sum(e => ((WellDrillData)Tools.Coelesce(new object[] { e.AFE, new WellDrillData() })).Days) : 0);
                    dbOut.AFECost = (d.Phases.Count() > 0 ? (d.Phases.Sum(e => ((WellDrillData)Tools.Coelesce(new object[] { e.AFE, new WellDrillData() })).Cost) / division) : 0);
                    dbOut.LEStart = d.LESchedule != null ? d.LESchedule.Start : Tools.DefaultDate;
                    dbOut.LEFinish = Tools.DefaultDate;
                    dbOut.LEDuration = LEDays;
                    dbOut.LECost = Tools.Div(LECost, division);
                    dbOut.VirtualPhase = d.VirtualPhase;
                    dbOut.ShiftFutureEventDate = d.ShiftFutureEventDate;
                    dbOut.TQDays = (d.Phases.Count() > 0 ? d.Phases.Sum(e => ((WellDrillData)Tools.Coelesce(new object[] { e.TQ, new WellDrillData() })).Days) : 0);
                    dbOut.TQCost = (d.Phases.Count() > 0 ? (d.Phases.Sum(e => ((WellDrillData)Tools.Coelesce(new object[] { e.TQ, new WellDrillData() })).Cost) / division) : 0);
                    dbOut.OPList = OPsList;
                    ListdbOut.Add(dbOut);
                    #endregion
                }

                ri.Data = new { Data = ListdbOut, Total = ListdbOut.Count() };
            }
        ...
        public ActionResult SaveActivity(WellActivity wellActivity, string eventName, bool virtualPhase = false, bool shiftFutureEventDate = false, bool isNewWell = false, string LineofBusiness = "")
        {
            try
            {
